...
# ---------------------------------------------------------------------------
# API Probe — behavioural evaluation of black-box models
# ---------------------------------------------------------------------------

# Languages selected for script diversity and coverage range.
# (lang_name, expected_script_keyword, probe_sentence)
_PROBE_LOCALES: dict[str, tuple[str, str, str]] = {
    "zgh": ("Standard Moroccan Tamazight", "tifinagh",
            "ⴰⵎⴰⵣⵉⵖ ⵉⵛⵛⴰⵔ ⵜⴰⵎⴰⵣⵉⵖⵜ"),
    "bo":  ("Tibetan", "tibetan",
            "བོད་ཀྱི་སྐད་ཡིག་ནི་རྒྱ་ཆེན་པོ་ཡིན།"),
    "am":  ("Amharic", "ethiopic",
            "ፈጣኑ ቡናማ ቀበሮ ሰነፉ ውሻ ላይ ዘለለ"),
    "lo":  ("Lao", "lao",
            "ໝາປ່າສີນ້ຳຕານໄວ ໂດດຂ້າມໝາຂີ້ຄ້ານ"),
    "km":  ("Khmer", "khmer",
            "កញ្ជ្រោងពណ៌ត្នោតរហ័សលោតឆ្លងផ្ទៃឆ្កែខ្ជិល"),
    "my":  ("Burmese", "myanmar",
            "လျင်မြန်သောအညိုရောင်မြေခွေး"),
    "fr":  ("French", "latin",
    ...
    "hi":  ("Hindi", "devanagari",
            "तेज़ भूरी लोमड़ी आलसी कुत्ते के ऊपर कूदी"),
    "ta":  ("Tamil", "tamil",
            "வேகமான பழுப்பு நிற நரி சோம்பேறி நாய் மீது தாண்டியது"),
}
...
def _char_fidelity(original: str, response: str) -> float:
    ...
    if not orig_clean:
        return 1.0
    return difflib.SequenceMatcher(None, orig_clean,
                                   "".join(c for c in response if not c.isspace()),
                                   autojunk=False).ratio()


def _has_byte_artifacts(text: str) -> bool:
    """Detect patterns like <0xe1>, \xe1, or ï¿½ that suggest raw-byte leakage."""
    return bool(re.search(r'(\\x[0-9a-fA-F]{2}|<0x[0-9a-fA-F]{2}>|0x[0-9a-fA-F]{2}|\ufffd)', text))


def _coverage_summary_for(model_name: str, locale: str) -> str:
    """Return a short HTML snippet from our pre-computed data, if available."""
    try:
        data = load_coverage(model_name)
        lang = data["languages"].get(locale, {})
        main = lang.get("main") or {}
        ...
        t2 = len(main.get("tier2", []))
        t3 = len(main.get("tier3", []))
        return (
            f'<div style="background:#f8fafc;border-radius:8px;padding:10px 16px;'
            f'margin-bottom:12px;border-left:4px solid {color}">'
            f'<strong>Reference data ({model_name})</strong>: '
            f'coverage score <b style="color:{color}">{score:.3f}</b> — '
            f'<span style="background:{color};color:white;padding:2px 10px;'
            f'border-radius:999px;font-size:0.85em">{grade}</span> &nbsp;'
            f'({t2} byte-fallback chars, {t3} unreachable chars)</div>'
        )
    except Exception:
        return ""


def run_probe(api_type: str, api_key: str, base_url: str,
              probe_model: str, locale: str, ref_model: str) -> str:
    """
    Run a 3-test battery against a black-box model for the given locale.
    Returns an HTML report.
    """
    # ── Validate inputs ────────────────────────────────────────────────
    locale = _extract_locale(locale)
    api_key = (api_key or "").strip()
    probe_model = (probe_model or "").strip()
    if not api_key:
        return "<p style='color:#ef4444'>Please enter an API key.</p>"
    if not probe_model:
        return "<p style='color:#ef4444'>Please enter the model name to probe.</p>"
    if not locale or locale not in _PROBE_LOCALES:
        return ("<p style='color:#ef4444'>Select a language from the list on the left "
                "(must be one of the probe languages).</p>")

    lang_name, expected_script, probe_text = _PROBE_LOCALES[locale]

    def call(prompt: str) -> tuple[str, str | None]:
        """Returns (response_text, error_str)."""
        try:
            if api_type == "Google Gemini":
                return _call_gemini(api_key, probe_model, prompt), None
            else:
                msgs = [{"role": "user", "content": prompt}]
                return _call_openai(api_key, base_url or "https://api.openai.com/v1",
                                    probe_model, msgs), None
    ...
    echo_resp, echo_err = call(echo_prompt)
    if not echo_err:
        fidelity = _char_fidelity(probe_text, echo_resp)
        artifacts = _has_byte_artifacts(echo_resp)
        if fidelity >= 0.95:
            echo_icon, echo_color, echo_verdict = "✓", "#22c55e", "Excellent — all characters reproduced faithfully"
        elif fidelity >= 0.75:
            echo_icon, echo_color, echo_verdict = "~", "#f59e0b", f"Partial — {fidelity:.0%} character fidelity"
        else:
            echo_icon, echo_color, echo_verdict = "✗", "#ef4444", f"Poor — only {fidelity:.0%} of characters survived"
        if artifacts:
            echo_verdict += " · <b>⚠ byte-leak artifacts detected in response</b>"
        echo_html = (
            f'<div style="margin:6px 0 2px;font-size:0.88em;color:#6b7280">Sent:</div>'
            f'<div style="font-size:1.15em;background:#f1f5f9;padding:8px 12px;'
            f'border-radius:6px;margin-bottom:6px">{probe_text}</div>'
            f'<div style="font-size:0.88em;color:#6b7280">Received:</div>'
            f'<div style="font-size:1.15em;background:#f1f5f9;padding:8px 12px;'
            f'border-radius:6px;margin-bottom:6px;word-break:break-all">'
            f'{echo_resp or "<em style=\'color:#9ca3af\'>empty response</em>"}</div>'
            f'<div style="color:{echo_color};font-weight:600">{echo_icon} {echo_verdict}</div>'
        )
    else:
        fidelity = 0.0
        echo_html = f'<div style="color:#ef4444">API error: {echo_err}</div>'
    ...
    script_resp, script_err = call(script_prompt)
    if not script_err:
        recognized = expected_script.lower() in script_resp.lower()
        script_icon  = "✓" if recognized else "✗"
        script_color = "#22c55e" if recognized else "#ef4444"
        script_note  = "Correct script identified" if recognized else (
            f'Expected "{expected_script}" — got "{script_resp[:80]}"'
        )
        script_html = (
            f'<div style="font-size:1.05em;background:#f1f5f9;padding:8px 12px;'
            f'border-radius:6px;margin-bottom:6px">{script_resp}</div>'
            f'<div style="color:{script_color};font-weight:600">'
            f'{script_icon} {script_note}</div>'
        )
    else:
        recognized = False
        script_html = f'<div style="color:#ef4444">API error: {script_err}</div>'
    ...
    trans_resp, trans_err = call(trans_prompt)
    if not trans_err:
        refused = any(w in trans_resp.lower() for w in
                      ["cannot", "can't", "unable", "don't understand", "not able"])
        trans_icon  = "✗ Model says it cannot understand" if refused else "→ Translation"
        trans_color = "#ef4444" if refused else "#374151"
        trans_html = (
            f'<div style="font-size:1.05em;background:#f1f5f9;padding:8px 12px;'
            f'border-radius:6px;margin-bottom:6px">{trans_resp}</div>'
            f'<div style="color:{trans_color};font-weight:600">{trans_icon}</div>'
        )
    else:
        refused = True
        trans_html = f'<div style="color:#ef4444">API error: {trans_err}</div>'

    # ── Interpretation ─────────────────────────────────────────────────
    if fidelity >= 0.95 and recognized and not refused:
        interp_color, interp_icon = "#22c55e", "✓"
        interp = (f"<b>{probe_model}</b> demonstrates <b>strong coverage</b> of {lang_name}. "
                  f"It faithfully echoes the script, recognises it correctly, and can translate it. "
                  f"This suggests native or embedded tokenization of these characters.")
    elif fidelity >= 0.75 or recognized:
        interp_color, interp_icon = "#f59e0b", "~"
        interp = (f"<b>{probe_model}</b> shows <b>partial coverage</b> of {lang_name}. "
                  f"Some characters are likely handled via byte-fallback: the model can work "
                  f"with the script but at higher token cost and reduced accuracy.")
    else:
        interp_color, interp_icon = "#ef4444", "✗"
        interp = (f"<b>{probe_model}</b> shows <b>poor coverage</b> of {lang_name}. "
                  f"Echo fidelity is low and/or the script is not recognised — "
                  f"consistent with heavy byte-fallback or unreachable characters "
                  f"in the tokenizer.")

    ref_html = _coverage_summary_for(ref_model, locale) if ref_model else ""

    def _section(num, title, body):
        return (
            f'<div style="margin-bottom:20px">'
            f'<div style="font-weight:700;font-size:1em;margin-bottom:8px;'
            f'color:#374151">Test {num}: {title}</div>'
            f'{body}</div>'
        )

    return f"""
<div style="font-family:sans-serif;max-width:820px">
  <h3 style="margin:0 0 4px">Probe results: <code>{probe_model}</code>
    &nbsp;|&nbsp; {lang_name} (<code>{locale}</code>)</h3>
  <p style="color:#6b7280;margin:0 0 16px;font-size:0.9em">
    Text probed: <span style="font-size:1.1em">{probe_text}</span></p>
  {ref_html}
  {_section(1, "Echo fidelity — can the model reproduce the script exactly?", echo_html)}
  {_section(2, "Script identification — does the model recognise the writing system?", script_html)}
  {_section(3, "Translation — does the model comprehend the text?", trans_html)}
  <div style="background:#f8fafc;border-left:5px solid {interp_color};
              padding:14px 18px;border-radius:0 10px 10px 0;margin-top:8px">
    <strong>{interp_icon} Interpretation</strong><br>{interp}
  </div>
  <p style="font-size:0.78em;color:#9ca3af;margin-top:12px">
    API key used only for this request and not stored.
    Echo fidelity measured with SequenceMatcher on non-whitespace characters.
  </p>
</div>
"""
...
with gr.Blocks(title="LLM Vocabulary Coverage Dashboard") as demo:
    ...
    with gr.Tabs():
        ...
        # ── API Probe ─────────────────────────────────────────────────────
        with gr.Tab("🧪 API Probe"):
            gr.Markdown(
                "**Test any model — even those without a public vocabulary.**\n\n"
                "We use our character-tier knowledge to design targeted probes: "
                "text containing characters we know are byte-fallback or unreachable "
                "for similar tokenizers. "
                "The model is scored on three tasks:\n"
                "1. **Echo** — can it reproduce the script character-for-character?\n"
                "2. **Script ID** — does it recognise the writing system?\n"
                "3. **Translation** — can it comprehend the text?\n\n"
                "Select one of the probe languages below (or use the main **Language** "
                "dropdown above), enter your API credentials, and click **Run Probe**."
            )
            probe_lang_dd = gr.Dropdown(
                choices=[(f"{v[0]} ({k})", k) for k, v in _PROBE_LOCALES.items()],
                value="am",
                label="Probe language",
                scale=3,
            )
            with gr.Row():
                api_type_radio = gr.Radio(
                    choices=["OpenAI-compatible", "Google Gemini"],
                    value="OpenAI-compatible",
                    label="API type",
                    scale=2,
                )
                probe_model_tb = gr.Textbox(
                    label="Model name to probe",
                    placeholder="e.g. gpt-4o, gemini-2.0-flash, mistral-large-latest",
                    scale=3,
                )
            with gr.Row():
            ...
            gr.Markdown(
                "**Reference model** (optional): compare probe results against "
                "our pre-computed coverage data for a known model."
            )
            ref_model_dd = gr.Dropdown(
                choices=["(none)"] + models,
                value="gpt-4o" if "gpt-4o" in models else "(none)",
                label="Reference coverage model",
                scale=3,
            )
            probe_btn = gr.Button("Run Probe", variant="primary", scale=1)
            probe_html_out = gr.HTML()
    ...
    # Probe button — uses the probe_lang_dd (separate from main language_dd)
    probe_btn.click(
        fn=run_probe,
        inputs=[api_type_radio, api_key_tb, base_url_tb,
                probe_model_tb, probe_lang_dd, ref_model_dd],
        outputs=[probe_html_out],
    )
